...
private struct GoalCoachCard: View {
    ...
    /// Persisted JSON envelope of the last plan we generated for this goal +
    /// scored day. Survives app close/reopen so the user doesn't get a new
    /// briefing on every launch — only when goal, date, or AI toggle change.
    /// The taskKey is encoded inside the JSON so we can verify the cached
    /// plan still applies to the current inputs before restoring it.
    @AppStorage("goalCoach.storedPlan") private var storedPlanJSON: String = ""
    ...
    /// Codable envelope persisted to UserDefaults. Stores the cache key
    /// alongside the plan fields so we can verify the cached entry still
    /// matches the current inputs (goal, scoreDate, AI toggle) before restore.
    ...
    var body: some View {
        ...
        .task(id: taskKey) {
            // 1) in-memory cache: visited this tab earlier in the session?
            if generatedForKey == taskKey, case .ready = phase { return }
            // 2) on-disk cache: fresh app launch but the same goal+day? Restore
            //    so the user reads the *same* briefing they saw before instead
            //    of a new model-generated take. Only the manual regenerate
            //    button bypasses this.
            if let stored = loadStoredPlan(), stored.key == taskKey {
                usedFallback = stored.usedFallback
                generatedForKey = taskKey
                withAnimation {
                    phase = .ready(headline: stored.headline,
                                   focus: stored.focus,
                                   steps: stored.steps,
    ...
    private func generate() async {
        ...
        do {
            ...
            generatedForKey = taskKey
            saveStoredPlan(StoredPlan(key: taskKey,
    ...
    private func loadStoredPlan() -> StoredPlan? {
        guard !storedPlanJSON.isEmpty,
              let data = storedPlanJSON.data(using: .utf8) else { return nil }
        return try? JSONDecoder().decode(StoredPlan.self, from: data)
    }

    private func saveStoredPlan(_ plan: StoredPlan) {
